...
a configured Vault server. The options include an Identity Entity with an associated
Userpass account for exploring the identity system. It will also enable an App Role
authentication mechanism for exploring a common method to authenticate applications.
The option also seeds the system with the required policies and a sample secret. More
information can be found in the banner found on the CLI when the flag is toggled.
